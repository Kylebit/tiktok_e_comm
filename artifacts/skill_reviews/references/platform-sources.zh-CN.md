...
取消订单总数必须与“取消且有结算/退款”和“取消且无结算”两个子集分开。TikTok 下单月份月报按官方 `order_created_at` 筛选已结算订单；真实广告只取自然月内 `GMV Payment for TikTok Ads` 的 Finance 扣款并按买家商品 GMV 分摊。带 `related_order_id` 的物流赔付等调整要回挂原订单并保留双方源事实。
