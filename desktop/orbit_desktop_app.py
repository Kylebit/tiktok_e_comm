...
from tkinter import BOTH, END, LEFT, RIGHT, Tk, X
...
class OrbitDesktopApp:
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(
            header,
            text="把 Orbit OS、Orbit Treasury 和 Orbit Rus 放进同一个轻量桌面壳里。",
    ...
    def _build_module_tab(self, tab: ttk.Frame, spec: ModuleSpec) -> None:
        ...
        for idx, (label, url) in enumerate(spec.quick_links):
            ttk.Button(btn_row, text=label, command=lambda u=url: webbrowser.open(u)).pack(side=LEFT, padx=(8 if idx else 0, 0))
    ...
    def _stop_module_blocking(self, spec: ModuleSpec) -> None:
        proc = spec.process
    ...
    def stop_module(self, spec: ModuleSpec) -> None:
        threading.Thread(target=self._stop_module_async, args=(spec,), daemon=True).start()

    def _stop_module_async(self, spec: ModuleSpec) -> None:
        self._stop_module_blocking(spec)
        if not self._closing:
            self.root.after(0, self.refresh_all_now)

    def restart_module(self, spec: ModuleSpec) -> None:
        threading.Thread(target=self._restart_module_async, args=(spec,), daemon=True).start()

    def _restart_module_async(self, spec: ModuleSpec) -> None:
        self.append_log(spec.key, f"[restart] {spec.title}")
        self._stop_module_blocking(spec)
        time.sleep(0.25)
        if not self._closing:
            self.root.after(0, self.start_module, spec)
    ...
    def stop_all(self) -> None:
        threading.Thread(target=self._stop_all_async, daemon=True).start()
    ...
    def _refresh_all_worker(self) -> None:
        snapshot: list[tuple[ModuleSpec, bool, str, bool]] = []
        for spec in MODULES:
            ok, detail = self._fetch_health(spec.health_url)
            running = spec.process is not None and spec.process.poll() is None
            snapshot.append((spec, ok, detail, running))
        self.root.after(0, self._apply_refresh_snapshot, snapshot)

    def _apply_refresh_snapshot(self, snapshot: list[tuple[ModuleSpec, bool, str, bool]]) -> None:
        for spec, ok, detail, running in snapshot:
            state = "running" if running else "stopped"
            if ok:
                self.status_vars[spec.key].set(f"在线 · {spec.url}")
            else:
                self.status_vars[spec.key].set(f"离线 · 进程 {state} · {detail}")
    ...
    def refresh_all_now(self) -> None:
        with self._refresh_lock:
            if self._closing or self._refresh_inflight:
                return
