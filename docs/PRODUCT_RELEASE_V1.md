...
7. 付费生图前仍有独立预检和明确的费用确认。经验分镜自动采用不等于自动付费。
8. 每个真实生成版本仍必须人工选择批准、返工或拒绝。只有技术验证通过且人工批准的当前版本可以进入最终图片集。
...
- 内容审核包已存在；今天 15:11 的 AI 规划成功返回 `sc1` 场景图与 `sp1` 卖点图两个分镜，已无费用迁移为 `experience_recipe_auto_v1`；
- 两张图的本地生成前检查已完成，状态为 `ready_for_explicit_paid_confirmation`；当前仍是已生成 0、付费任务 0，正式页明确停在“等待 Kyle 确认付费”；
