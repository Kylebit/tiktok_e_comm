...
| 正式英文标题与平台候选 | 内容域 ToAPI 文本模型 + 渠道规则 | 采用/编辑英文母版并审核平台候选 | ToAPI `gpt-5.4-mini-official` 按平台重写本地候选；不是逐字翻译，不直接写妙手或渠道 |
...
8. 中文来源标题保持独立只读。`AI 生成平台标题` 使用中文来源、类目、尺寸、保留规格和已验证属性调用 ToAPI `gpt-5.4-mini-official`，不是逐字翻译，而是按 TikTok/Shopee/Ozon 的语言、搜索词序、移动端可读性与长度限制分别优化；结果保存模型、规则版本、事实签名和时间，但不会自动成为商品事实或平台草稿。
...
当前 production registry 已接入 TikTok、Shopee 与 Ozon 的统一 Adapter。正式按钮只执行不可变计划里的已选目标；每个目标保存独立幂等键、提交回执和回读证据。已有 SKU 与计划不一致时，Adapter 只允许原地修复明确可修字段，禁止再次创建商品；价格、图片、身份或状态不一致仍会阻断并等待人工处理。

### Shopee CNSC 实际链路

1. 内容域先形成一份已批准的英文语义母版和 500–3000 字符的事实型英文描述。
2. 渠道域只创建一份 Shopee CNSC 全球商品；新单规格商品在发布国家站点前建立一个显式 `Model SKU`。旧全球商品若已用商品级 `item_sku` 发布，不做破坏性变体迁移。
3. 国家发布任务只提交全球商品 ID、目标店、价格、状态和物流；不再把英文标题/描述覆盖到 TH、VN 等本地商品，交给 Shopee 的全球商品导入流程自动翻译。PH、MY 可以保留英文。
4. 官方 API 回读检查商品级 SKU 或 Model SKU、站点语言、描述完整度、价格、图片、状态和物流。
5. 若历史商品跳过了自动翻译，只对原 item ID 做 ToAPI 本地化和 `update_item` 原地修复；绝不为文案问题再次发布同一 SKU。
6. 物流渠道逐条尝试开启。平台因重量、尺寸、路线或店铺资格拒绝的渠道保存拒绝原因，其余渠道全部开启；一个不适用的自提柜/即时配送渠道不会使整次同步失败。
...
flowchart LR
    ...
    CHANNELS --> CNSC["Shopee CNSC 英文母版\n国家导入自动翻译 / 原地修复"]
...
## Shopee 0953 回归事实（2026-07-27）

- CNSC 全球商品 `42433015689` 已回读英文标题、657 字符结构化描述，以及 `PVC / Dog / Quantity 1` 事实属性。
- PH `49464903906`、MY `50364884140` 保持英文；TH `48964906224`、VN `29595408894` 已使用本地语言。四站描述长度分别为 657、657、676、691。
- 四店官方 API 全目录扫描中，Seller SKU `0953` 每店精确 1 个商品，没有由修复产生重复项。
- 0953 是历史单规格商品，当前 SKU 位于商品级且 `has_model=false`。这不影响库存、价格、订单或跨平台对齐；为避免 Shopee 要求重建四店模型价格，本次不做破坏性迁移。新商品改为发布前建立显式 Model SKU。
- PH 可见 3 个物流渠道全部开启；MY/TH/VN 的跨境标准、快递、海运等可用渠道全部开启。剩余关闭项均已被 Shopee 官方接口以当前包裹重量/尺寸超出渠道范围拒绝，主要是自提柜与即时配送。
...
- 新品标题生成入口使用 ToAPI `gpt-5.4-mini-official`：从已验证中文事实生成英文语义母版，再分别按 TikTok、Shopee 和 Ozon 的语言、搜索词序、移动端可读性与字段上限重写候选；不是机械翻译，也不会直接写妙手或渠道。
- 商品标题仍含中文、或人工采购成本与单一来源规格价不一致时，系统显示审批提醒但允许 Kyle 锁定当前 revision；缺少标题、非正成本/重量/包装尺寸、规格证据无效、SKU 冲突等仍是硬阻塞。
- 没有 ProductApproval 或 ReleasePlan；渠道发布保持禁用。

ToAPI 标题候选与人工采购成本差异都会在审批区显示。Kyle 可以先采用模型候选或继续使用当前标题，也可以保留人工核对后的 `9 CNY`；标题含中文和 `9 CNY vs 8.1 CNY` 都是可确认提醒，不会禁用“批准并锁定”动作。锁定后这些提醒进入审批审计；缺少标题、非正成本/重量/尺寸、无效来源规格或 Seller SKU 冲突仍会阻止审批。
