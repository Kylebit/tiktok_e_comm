...
8. 编辑保存必须提交当前 `expected_revision`；后端校验标题、正数成本、正数重量、三个正数尺寸，以及所选规格确实属于当前来源。
9. 保存成功只建立下一本地 revision；页面必须使用该响应整体重绘商品事实、全部国家/店铺售价、费用审计和渠道预检，不能只替换表单数字。
10. 浏览器持有旧 revision、规格被来源移除或事实已被审批锁定时，保存返回明确冲突，不覆盖较新的事实。
11. AI 图片工作室始终可在新标签页并行打开；它负责内容版本，不负责首次商品建档。
