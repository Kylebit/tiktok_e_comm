...
| 采购成本、重量、包装尺寸 | 上游来源 + 工作台审核值 | 在正式页修改并最终批准 | 同页编辑，保存为新 revision 并重算售价/预检 |
...
| 4. 核对商品事实 | 在正式页编辑标题、真实规格、成本、重量、包装长宽高并保存 | 上游事实、当前 revision 和人工修改 | 下一 ProductFacts revision，同时刷新售价与渠道预检 | 工作台 JSON 原子写入 | 无 |
...
6. 正式 dashboard 即使还没有图片审核包也必须打开：内容阶段显示待审核 blocker，但商品事实、自动 Seller SKU、主图预览和编辑面板可正常工作。
7. 编辑保存必须提交当前 `expected_revision`；后端校验标题、正数成本、正数重量、三个正数尺寸，以及所选规格确实属于当前来源。
8. 保存成功只建立下一本地 revision，然后重新计算全部目标售价与发布预检。
9. 浏览器持有旧 revision、规格被来源移除或事实已被审批锁定时，保存返回明确冲突，不覆盖较新的事实。
10. AI 图片工作室始终可在新标签页并行打开；它负责内容版本，不负责首次商品建档。
...
该采集箱已通过正式商品发布中心完成初始化：

- 本地妙手来源缓存已由只读详情接口重新建立；
- 本地商品工作台为 revision 1；
- 来源商品为 `BB6102` 水彩花卉蝴蝶墙贴，正式页已显示来源主图；
- 系统自动 Seller SKU 候选为 `0952`，Kyle 无需也不能手填；
- 标题、成本、重量、包装尺寸和两条来源规格已进入同页编辑面板；
- 当前来源规格包含一条“咨询客服定制”占位项，并存在 `0.2 / 8.1 CNY` 多价格冲突，因此商品事实审批保持禁用；
- 本地 AI 图片套图尚未重新建立，内容阶段保持待审核；
- 没有 ProductApproval、ContentApproval 或 ReleasePlan；
- 妙手同步和全部渠道发布保持禁用。

重复调用首次采集接口已验证为幂等：不会再次读取妙手，不改变 revision 或状态文件摘要，也没有任何外部写入。下一步应由 Kyle 在正式页只保留真实可采购规格、核对正确采购成本并编辑标题；保存后会形成 revision 2。内容与图片工作室可同时在另一标签页完成原素材或 AI 辅助路线。
