...
_SHOPEE_DATA = re.compile(r"const DATA = (.*?);(?:\r?\n|</script>)", re.DOTALL)
...
def adapt_local_profit_snapshots(
    ...
    for value in paths:
        ...
        result = adapt_profit_snapshot_text(text, source_name=path.name, source_updated_at=updated, costs_by_sku=costs_by_sku, reporting_period=reporting_period)
    ...
    checksum = _checksum({"sources": sources, "rows": all_rows})
...
def discover_local_profit_snapshots(
    ...
    return adapt_local_profit_snapshots(paths, costs_by_sku=costs_by_sku, reporting_period=reporting_period)


def adapt_profit_snapshot_text(
    text: str, *, source_name: str, source_updated_at: str,
    costs_by_sku: Mapping[str, object] | None = None, reporting_period: tuple[date, date] | None = None,
) -> LocalSnapshotAdaptation:
    ...
    if source_name.lower().endswith(".csv") and _TIKTOK_REGION.search(source_name):
        rows, issues, raw, rejected = _tiktok_rows(text, source_name, source_updated_at, costs_by_sku or {}, reporting_period)
...
def _tiktok_rows(text: str, source: str, updated: str, costs: Mapping[str, object], period: tuple[date, date] | None):
    ...
    for index, item in enumerate(csv.DictReader(io.StringIO(text))):
        ...
        sku = _text(item.get("SKU ID")); occurred = _text(item.get("Statement Date"))
        ...
        rows.append(_row("tiktok", region, order, sku, currency, amount, cost, occurred, updated, source))
...
def _shopee_rows(text: str, source: str, updated: str, period: tuple[date, date] | None):
    ...
    for index, item in enumerate(data.get("rows") or []):
        ...
        order, sku = value("Order SN"), value("SKU")
        region = _text(item.get("region")); currency = _text(item.get("currency") or value("Currency"))
        occurred = value("Release Time") or value("Purchase Date")
        ...
        rows.append(_row("shopee", region, order, sku, currency, amount, cost, occurred, updated, source))
    return rows, issues, raw, rejected


def _row(channel, region, order, sku, currency, amount, cost, occurred, updated, source):
    return {"channel": channel, "region": region, "order_id": order, "sku_id": sku, "currency": currency.upper(), "settlement_amount": amount, "cost_cny": cost, "occurred_at": occurred, "source_updated_at": updated, "calculation_kind": "realized", "source_file": source}
...
def _within(value, period):
    ...
    try: current = date.fromisoformat(value[:10])
