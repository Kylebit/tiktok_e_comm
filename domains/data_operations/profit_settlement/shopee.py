...
def build_weekly_report(
    ...
    *,
    ...
    for index,row in enumerate(source_rows):
        ...
        quantity=_decimal(row.get("quantity"));settlement=_decimal(row.get("net_settlement_amount"));paid=_decimal(row.get("product_sales_amount") or row.get("buyer_paid_product_amount"));cash_paid=_decimal(row.get("buyer_cash_paid_product_amount"));invalid=False
...
def build_monthly_report(
    ...
    *,
    ...
    for index, row in enumerate(source_rows):
        ...
        if fees is None:
            rejected += 1; continue
        prepared.append({"row": row, "record_id": record_id, "occurred": _datetime(row.get("occurred_at")), "settled_at": settled_at, "sku": sku, "cost": cost, "currency": currency, "fx_rate": fx_rate, "quantity": quantity, "settlement": settlement, "paid": paid, "cash_paid": cash_paid, "paid_cny": paid * fx_rate, "fees": fees, "external": external})
