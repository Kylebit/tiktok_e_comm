...
def build_weekly_report(
    ...
    *,
    ...
    for index, row in enumerate(source_rows):
        ...
        quantity = _decimal(row.get("quantity")); settlement = _decimal(row.get("net_settlement_amount"))
        paid = _decimal(row.get("product_sales_amount") or row.get("buyer_paid_product_amount"))
        cash_paid = _decimal(row.get("buyer_cash_paid_product_amount")); ad_basis = cash_paid if cash_paid is not None else paid; invalid = False
        for missing, field, code in (
            ...
            (ad_basis is None, "buyer_cash_paid_product_amount", "missing_ad_basis"),
        ):
            if missing:
                issues.append(_issue(code, record_id, field)); invalid = True
        if invalid:
            rejected += 1
            continue
        ...
        settlement_cny = settlement * fx_rate; product_cost = cost.unit_cost_cny * quantity
        ad_local = ad_basis * rate_value; ad_cny = ad_local * fx_rate
        lines.append({
            ...
            "occurred_at":_datetime(row.get("occurred_at")),"settled_at":settled_at,"settlement_status":"settled","settlement":{"currency":currency,"net_amount_local":settlement,"net_amount_cny":settlement_cny,"product_sales_amount_local":paid,"buyer_cash_paid_product_amount_local":cash_paid,"buyer_paid_product_amount_local":paid},"fx":{"rate_cny_per_local":fx_rate,**fx.payload()},
            ...
            "advertising":{"mode":"estimated_rate","rate":rate_value,"input_source":_text(ad_rate_source) or ("default_22" if rate_value == Decimal("0.22") else "operator_global_override"),"policy_version":"operator-adjustable-ad-rate/v1","basis":"buyer_cash_paid_product_amount","basis_amount_local":ad_basis,"amount_local":ad_local,"amount_cny":ad_cny},
...
def build_monthly_report(
    ...
    *,
    ...
    for index, row in enumerate(source_rows):
        ...
        sku = _text(row.get("canonical_sku")); cost = costs.get(sku)
        currency = _text(row.get("currency")).upper(); fx_rate = fx.get(currency)
        quantity = _decimal(row.get("quantity")); settlement = _decimal(row.get("net_settlement_amount")); paid = _decimal(row.get("product_sales_amount") or row.get("buyer_paid_product_amount")); cash_paid = _decimal(row.get("buyer_cash_paid_product_amount"))
        invalid = False
        for missing, field, code in (
            (not sku or cost is None, "canonical_sku", "missing_cost"),
            (not currency or fx_rate is None, "currency", "missing_fx"),
        ...
        if fees is None:
            rejected += 1; continue
        fulfillment = _fulfillment(row, record_id, issues)
        prepared.append({"row": row, "record_id": record_id, "occurred": _datetime(row.get("occurred_at")), "settled_at": settled_at, "sku": sku, "cost": cost, "currency": currency, "fx_rate": fx_rate, "quantity": quantity, "settlement": settlement, "paid": paid, "cash_paid": cash_paid, "paid_cny": paid * fx_rate, "fees": fees, "external": external, "fulfillment": fulfillment})
...
def _advertising(value: Mapping[str, object] | None, issues: list[ShopeeQualityIssue]) -> dict[str, Any] | None:
    ...
    if total is None or total < 0 or not source or not as_of or not snapshot:
        ...
        return None
    return {"mode": "allocated_actual_ads", "total_cny": total, "source": source, "as_of": as_of, "snapshot_id": snapshot, "allocation_basis": "product_sales_amount_cny", "allocation_version": "gmv-pro-rata-v1"}
...
def _line(item: Mapping[str, Any], advertising_cny: Decimal, costs: CostSnapshot, fx: FxSnapshot, advertising: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "settlement": {"currency": item["currency"], "net_amount_local": item["settlement"], "net_amount_cny": settlement_cny, "product_sales_amount_local": item["paid"], "buyer_cash_paid_product_amount_local": item["cash_paid"], "buyer_paid_product_amount_local": item["paid"]},
        ...
        "advertising": {**advertising, "basis": "product_sales_amount_cny", "basis_amount_cny": item["paid_cny"], "amount_cny": advertising_cny},
...
def _fulfillment(row: Mapping[str, object], record_id: str, issues: list[ShopeeQualityIssue]) -> dict[str, Any]:
...
def _allocate_order_cost(total: Decimal, lines: list[dict[str, Any]]) -> list[Decimal]:
    ...
    weights = [
        max(
            (_decimal((line.get("settlement") or {}).get("product_sales_amount_local")) or Decimal("0"))
    ...
    for index, weight in enumerate(weights):
        amount = total - assigned if index == len(lines) - 1 else total * weight / denominator
        result.append(amount)
        assigned += amount
    return result


def _fulfillment_order_counts(lines: Iterable[Mapping[str, Any]]) -> dict[str, int]:
    orders: dict[str, str] = {}
    for line in lines:
        ...
        if order_id:
            orders[order_id] = _text(fulfillment.get("mode")) or "unknown"
    return {
