...
8. For weekly reports, default each platform's advertising fraction to `0.22` (22%) unless the user supplies an explicit global or platform override. Apply it to buyer-paid product amount after seller discount, excluding buyer shipping. Preserve the rate, input source (`default_22`, global override, or platform override), policy version, and basis as an estimate in every report; never label it as actual spend. Exclude an actual TikTok advertising adjustment from the weekly order-settlement basis before applying the estimate, and reconcile the exclusion separately so advertising is never deducted twice.
...
11. Generate one independent JSON/HTML report per platform. For Shopee, read official order `create_time`, retain it as `order_created_at`/`occurred_at`, and display it independently from settlement time; never substitute release time for missing order time. Consolidate multiple same-period settlement facts for one order line only when product identity and quantity agree and no more than one fact carries a positive advertising basis; otherwise fail closed. Sum settlement and fee facts, charge product cost and estimated advertising once, and retain every source fact identity/time in JSON and HTML. Sort order lines by settlement timestamp descending; break equal timestamps deterministically by order ID and order-line ID. Follow the legacy order-profit table pattern: render order/product/quantity, buyer-paid amount, settlement, live FX rate/provider/as-of time, unit and total cost, advertising basis/rate/input-source/amount, external cost, profit, margin, and every fee component as its own column. Display the FX rate at eight decimal places so small ratios remain meaningful; show price and cost values at two decimal places while retaining full Decimal precision and FX snapshot identity in JSON. Give wide HTML tables synchronized horizontal scrollbars above and below the order rows so every column remains reachable without scrolling to the final row. Review totals, every quality issue and assumption warning, negative-profit lines, fee inclusion semantics, source counts, settlement reconciliation, cost conflicts, and rejected rows.
