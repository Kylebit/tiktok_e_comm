...
Classify fulfillment from escrow-detail finance evidence. Accept `seller_shipping_fee` or the API-equivalent `actual_shipping_fee` together with `shipping_fee_sst`/`sst`. Only two exact zeros mean local fulfillment; any non-zero field means cross-border. Missing either field is a quality issue, never an implied local order. Apply the configured local shipping and warehouse charge once per parent order, not once per expanded item line.
