...
For TikTok Thailand, determine the customs route from settled Finance tax components, not from the order fulfillment operator. Preserve `fee_tax_breakdown.tax.import_vat_amount` and `fee_tax_breakdown.tax.customs_duty_amount` from the current Finance schema, or their exact top-level aliases when the live response still exposes them. A non-zero value in either component is cross-border evidence; two present exact zeros mean local. Missing either field is blocking and must never be converted to zero. Apply the configured local-fulfillment cost once per local parent order (default CNY 4), not once per expanded item line, and retain its deterministic allocation and policy lineage. Do not retain `fulfillment_type`, `delivery_type`, `shipping_type`, `delivery_option_name`, or `warehouse_id` in settlement/report artifacts.

For TikTok Philippines, preserve the top-level Finance `shipping_cost_amount` as normalized `merchant_shipping_fee`. Exact zero means local fulfillment and any non-zero amount means cross-border. Keep `actual_shipping_fee_amount` and `platform_shipping_fee_discount_amount` as separate financial components but never use either as the route predicate: the actual-shipping component can be non-zero while the merchant shipping fee shown by Finance is zero. Missing `shipping_cost_amount` is blocking rather than local-by-default.

Use `/finance/202501/statements/{statement_id}/statement_transactions` and read the `transactions` collection. TikTok deprecated the v202309 statement-transaction endpoint; do not fall back to it to make a report pass. Treat any legacy or partial response that omits the two tax components as incomplete evidence rather than a local order.
