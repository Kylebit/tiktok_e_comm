...
For Shopee, classify local fulfillment only when seller/actual shipping fee and shipping SST are both exactly zero. A non-zero value means cross-border; a missing field is blocking. Local orders default to CNY 4 shipping plus CNY 4 warehouse cost per parent order, both operator-adjustable. Allocate each parent-order cost deterministically across its item lines, retain the two allocations separately, include them in `external_costs_cny`, and include the policy inputs in the idempotency fingerprint.
