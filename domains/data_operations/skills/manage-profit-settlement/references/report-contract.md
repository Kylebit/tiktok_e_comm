...
- Shopee fulfillment mode and its official shipping-fee/SST evidence; for local orders, separate allocated CNY shipping and warehouse costs plus their per-order policy
...
For Shopee, classify fulfillment from the official import-tax fields. `vat_on_imported_goods` or import-duty (`th_import_duty`, with explicit regional aliases) being non-zero is direct cross-border evidence. Classify local only when both fields are present and exactly zero. A missing field is blocking; an asymmetric pair remains cross-border but produces `incomplete_cross_border_tax_pair` and cannot be ready. Local orders default to CNY 4 shipping plus CNY 4 warehouse cost per parent order, both operator-adjustable. Allocate each parent-order cost deterministically across its item lines, retain the two allocations separately, include them in `external_costs_cny`, and include the policy inputs in the idempotency fingerprint.

Order lines are initially sorted by settlement timestamp descending, with order ID and order-line ID as deterministic ascending tie-breakers. HTML is a display projection: let the order-created header toggle ascending and descending order while keeping missing times last and breaking ties by order ID/order-line ID; do not mutate the JSON audit order. Show only Seller SKU, put net settlement CNY, total product cost CNY, advertising CNY, profit, and margin immediately after it, and put product name at the far right. Show the fulfillment type and separate local shipping/warehouse cost columns. Show the country/region code without the internal shop identifier, render a valid HTTPS main-image URL as an image, show the live CNY-per-local FX rate, provider and provider as-of time as independent columns, every platform fee component as an independent column, FX rates at eight decimal places, and all price/cost values at exactly two decimal places. Wide tables expose synchronized horizontal scrollbars above and below the order rows. JSON remains the audit artifact and retains full Decimal precision plus FX snapshot identity.
