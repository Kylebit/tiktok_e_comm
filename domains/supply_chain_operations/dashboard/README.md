...
所有批次只使用雅仓日志 `已入库（Reach the domestic warehouse）` 作为运输起算点。`expected_sellable_date = reached_domestic_warehouse_date + country_transport_days + 2 days`。建单、标记发货、核验、装柜和报关时间都不能替代已入库时间；缺少已入库日志时，日期和供应事件保持阻断。截图确认的泰国批次 `THML4038-58701` 已入库时间为 `2026-08-04 15:39:15`，据此估算可售日为 `2026-08-21`。

在途采用分时点投影，不再视为今天已经到仓：先用当前可用库存满足日需求；到某批预计可售日期时才加入该批在途数量；随后继续消耗到本次新补货的预计可售日期。当前系统估算以雅仓入库批次的“标记发货”时间为优先锚点；未取得标记发货时间时，使用入库单创建日回退并明确标注。运输周期沿用 MY/PH 25 天、TH/VN 15 天，另加 2 天签收上架缓冲。泰国两批在途在缺少 SKU 级批次拆分时使用较晚一批的日期，避免过早抵扣。
...
- 建议件数只依赖需求、可用库存、带预计可售日期的在途和国家补货周期。缺尺寸时体积显示待补充，缺重量时本土处理和收益显示待补充，缺成本时预计占款显示待补充；这些资料不再阻断建议件数。
