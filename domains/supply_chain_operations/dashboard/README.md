...
国家海外运输周期使用雅仓完成批次的保守历史模型作为决策参考：排除雅仓标记异常的批次和没有正式头程物流的记录；有效样本至少 5 批时采用创建到签收总天数的 nearest-rank P80，再减去已批准的 3 天备货和 4 天到国内仓；最终周期默认不得低于原业务周期，但用户明确批准的国家周期具有最高优先级。2026-08-09 只读快照共 21 个完成批次，排除 4 个，MY/TH/VN/PH 有效样本分别为 2/9/5/1。历史模型建议 TH 20 天，但用户最终确认 TH 固定为 15 天；当前海外运输周期为 MY 25、TH 15、VN 15、PH 25 天。
