...
“资料待补”的具体 SKU 会显示“手动补齐”入口，可填写包装长/宽/高、重量、采购成本及可选来源备注。在途按批次显示“修改批次时间”入口；一次覆盖完整批次的预计可售日期，并同步影响该批次内全部 SKU，不会改变其他批次。两类数据都只写入当前浏览器的 `localStorage`，保存后即时重算，并可随时清除恢复原始快照或系统估算。
