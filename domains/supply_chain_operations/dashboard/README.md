...
每个 SKU 同时展示两份双平台需求与到仓结果。日期权重口径沿用 7天60% + 第8–15天30% + 第16–30天10% 的趋势日均；30日实绩口径使用 TikTok 与 Shopee 完整30天有效订单件数之和除以30，不加权。两份结果共用相同现货、在途批次、预计可售日期、提前期和目标覆盖，分别计算提前期需求、到仓剩余、目标库存与建议件数，绝不把两份建议平均或合并成一个数字。
