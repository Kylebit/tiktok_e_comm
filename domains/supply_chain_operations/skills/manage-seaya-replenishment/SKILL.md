...
7. Calculate lead-time demand, time-phased arrival stock, target coverage, and recommended quantity from eligible orders plus Seaya supply. Read every inbound batch identity, its exact per-SKU quantity, and its `标记发货` operation-log time when available. Estimate each batch's sellable date from the country transit policy plus shelving buffer, and let a user override that complete batch date locally. Calculate fixed head freight, handling cost, and known savings from settlement economics. Apply the current user-approved head-freight policy of CNY 1 per unit to every country, site, and SKU.
