...
- For the newly recommended shipment, use `new_replenishment_lead_days = 7 preparation days + country transit days`. Apply the same full lead to demand consumption and the displayed new-goods expected-sellable date. Do not add this preparation period to existing Seaya inbound batches.
...
- Estimate `expected_sellable_at = effective_anchor_at + country_transit_days + shelving_buffer_days`, where effective anchor is actual inbound first and the explicit creation-plus-four-day estimate second. Keep verification, loading, customs, sign, and shelving dates distinct; none may replace the approved hierarchy.
