...
Every SKU decision also carries a second, unweighted order scenario:

- `u30 = tiktok_recent30_valid_order_units + shopee_recent30_valid_order_units`
- `v30 = u30 / 30`
- method `full_30_day_actual_v1`

Both channel values must be complete nonnegative valid-order facts for the same country, SKU, and 30-day window. If either is unavailable, mark this scenario unavailable; never substitute long-window or settlement units. The date-weighted scenario and full-30-day scenario remain separate and equally visible.
...
Apply the quantity formulas twice: once with date-weighted `v`, and once with unweighted `v30`. Both calculations use the same target-coverage policy, current available inventory, exact inbound events, and expected-sellable dates. Do not merge the two recommended quantities. A summary row remains visible when either recommendation is strictly greater than its threshold.

Every scenario exposes projection method `TIME_PHASED_BATCH_EVENTS_V1` and a chronological trace. A consumption step carries `from_date`, `to_date`, `days`, `ceil(velocity × days)`, stock before/after, and unmet demand. An inbound step carries exact `batch_id`, expected-sellable date, quantity, and stock before/after. This trace is generated by the shared projection function for every SKU and both demand scenarios, not by SKU-specific presentation code.
