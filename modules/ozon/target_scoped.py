...
def read_existing_product(*, offer_id: str) -> dict:
    ...
    if len(items) != 1:
        return {"checks": {}}
    item = items[0]
    return {"product_id": str(item.get("product_id") or ""), "checks": {"created": item.get("is_created") is True, "approved": str(item.get("status") or "").lower() in {"approved", "price_sent"}, "title": bool(item.get("name")), "price": bool(item.get("price")), "images": bool(item.get("images")), "stock_false": not bool(item.get("stocks"))}}
