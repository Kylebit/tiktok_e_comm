...
4. A size_card is allowed only when the source evidence or human review verifies dimensions. Its model-generated base must contain only the clean product on white with generous right and bottom whitespace: no text, numbers, arrows, guide lines, or measurement graphics. The complete dimension lines, double arrowheads, and verified English labels are added deterministically later. When the operator asks for visible numbers or arrows, describe both layers clearly and assign every measurement graphic to deterministic local post-processing.
