...
def prepare_miaoshou_draft(offer_id_or_url: str) -> dict[str, Any]:
    ...
    video_url = source.get("video", {}).get("url") if review.get("video_action") == "keep" else ""
    video_url_length = len(video_url or "")
    auto_fix: dict[str, Any] | None = None
    if video_url and video_url_length > 255:
        blockers.append(
            f"视频 URL 过长（当前 {video_url_length} 字，妙手限制 255 字）；可自动移除视频后继续"
        )
        auto_fix = {
            "type": "remove_video",
            "target": "mainImgVideoUrl",
            "current_length": video_url_length,
            "limit": 255,
            "next_video_action": "remove",
        }
