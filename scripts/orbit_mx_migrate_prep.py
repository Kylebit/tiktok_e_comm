"""Orbit Hive：MX 旧品迁移准备（dry-run，不发布）。"""
...
DEFAULT_KEYS = ["0810", "0811", "0814", "0823", "0829"]
...
def prep_one(mk: str, *, rate: float) -> dict:
    ...
    return {
        ...
        "list_mxn": q.list_price_ceil_mxn,
        "sale_mxn": round(q.sale_price_mxn, 2),
        "weight_kg": q.weight_kg,
        "package_cm": q.package_cm,
        "manual_dims": bool(known.get("l")),
        "publish_allowed": False,
        "note": "需 Orbit Hive approval 后才可真实发布",
    }


def main() -> int:
    ap = argparse.ArgumentParser(description="MX 迁移 dry-run（Orbit Hive）")
    ap.add_argument("keys", nargs="*", default=DEFAULT_KEYS, help="对齐码，默认 5 个待迁移 SKU")
    ap.add_argument("--json-out", default=str(ROOT / "data" / "mx_confirm" / "orbit_dry_run.json"))
    args = ap.parse_args()
    ...
    print(f">>> Orbit dry-run: {len(results)} SKU | ready={ready} need_collect={need} blocked={blocked}")
    for r in results:
