...
def test_platform_start_does_not_claim_acceptance_when_another_scope_is_dispatching(
    monkeypatch,
):
    ...
    class ControlStore:
        ...
        def start_explicit_batch(job_id, *, target_labels):
            assert job_id == "oneclick-job:test"
            assert target_labels == ("shopee:GLOBAL",)
            return {
                **ControlStore.ensure_job(),
                "batch_scope_targets": ["tiktok:MX"],
            }
    ...
    status, response = product_server._start_shopee_global_release(
        {"confirm_publish": True}
    )

    assert status == 409
    assert response["ok"] is False
    assert response["error"]["code"] == "platform_dispatch_in_progress"
    assert response["external_writes_performed"] == []
    assert wake_calls == []
