...
def request(target="shopee:MY"):
    ...
    command = ({"target_label": target, "operation_kind": kind, "region": target[-2:], "model_sku": "0954", "approved_master_digest": "master", "parcel": {"weight_kg": .12, "dimensions_cm": [40, 3, 3]}, "excluded_logistics_ids": [50052] if target.endswith("VN") else [], "local_original_price": 10, "item_status": "NORMAL"} if target.startswith("shopee") else {"target_label": target, "operation_kind": kind})
