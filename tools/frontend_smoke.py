...
TREASURY = "http://127.0.0.1:8766"


def fetch(path: str, timeout: int = 8, *, base: str | None = None) -> tuple[int, str, bytes]:
    url = path if path.startswith("http") else (base or BASE) + path
...
def main() -> int:
    ...
    ok &= check(
        "treasury page",
        lambda: f"{fetch('/new-product', base=TREASURY)[0]} bytes={len(fetch('/new-product', base=TREASURY)[2])}",
    )
    ok &= check(
        "catalog page",
    ...
    def new_product_preview() -> str:
        status, _, body = fetch("/api/new-product/preview?offer_id=967648348081", base=TREASURY, timeout=20)
        data = json.loads(body.decode("utf-8"))
        if status != 200 or not data.get("ok"):
            raise RuntimeError(data)
    ...
    ok &= check("treasury preview", new_product_preview)
